...
class HMAvroConverter extends GroupConverter {

  private final ParentValueContainer parent;
  protected ComparableHashMap<Keyword,Object> currentRecord;
  ...
  private IFn fieldKeyword = Clojure.var("abracad.avro.util", "field-keyword");


  public HMAvroConverter(MessageType parquetSchema, Schema avroSchema) {
    this(null, parquetSchema, avroSchema);
  }

  public HMAvroConverter(ParentValueContainer parent, GroupType
    ...
    for (Type parquetField: parquetSchema.getFields()) {
      ...
      final Keyword keyword = (Keyword) fieldKeyword.invoke(avroField);
      converters[parquetFieldIndex++] = newConverter(nonNullSchema, parquetField, new ParentValueContainer() {
        @Override
        void add(Object value) {
          HMAvroConverter.this.set(keyword, value);
        }
      });
    }
  ...
  private void set(Keyword key, Object value) {
    this.currentRecord.put(key, value);
  }
  ...
  public void start() {
    ...
    this.currentRecord = new ComparableHashMap<Keyword, Object>();
  }
  ...
  private void fillInDefaults() {
    for (Map.Entry<Schema.Field, Object> entry : recordDefaults.entrySet()) {
      ...
      this.currentRecord.put((Keyword) fieldKeyword.invoke(f), defaultValue);
    }
  }
 ...
 ComparableHashMap getCurrentRecord() {
  ...
  static final class MapConverter<V> extends GroupConverter {
    ...
    @Override
    public void start() {
      this.map = new ComparableHashMap<Keyword, V>();
    }
    ...
    final class MapKeyValueConverter extends GroupConverter {
      ...
      public MapKeyValueConverter(Type parquetSchema, Schema avroSchema) {
        keyConverter = new PrimitiveConverter() {
          @Override
          final public void addBinary(Binary value) {
            key = Keyword.intern(value.toStringUsingUTF8());
          }
        };
