 ...
 * this class directly.
 */
public class HMAvroWriteSupport extends WriteSupport<Map> {

  private static final String AVRO_SCHEMA = "parquet.avro.schema";

  private IFn resolveUnion = Clojure.var("abracad.avro.write", "resolve-union*");
  ...
  public HMAvroWriteSupport() {
    IFn require = Clojure.var("clojure.core", "require");
    require.invoke(Clojure.read("abracad.avro.write"));
    require.invoke(Clojure.read("abracad.avro.util"));
  }


  public HMAvroWriteSupport(MessageType schema, Schema avroSchema) {
    super();
    ...
    if (map.size() > 0) {
      ...
      for (Object key : map.keySet()) {
        if (key instanceof Keyword) {
          writeValue(keyType, keySchema, ((Keyword) key).getName());
        } else {
          writeValue(keyType, keySchema, (String) key);
        }
      }
  ...
  private void writeUnion(GroupType parquetSchema, Schema avroSchema,
    ...
    int avroIndex = Integer.parseInt(String.valueOf(resolveUnion.invoke(avroSchema, value)));
